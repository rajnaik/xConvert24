...
export const POST: APIRoute = async ({ request }) => {
  ...
  const { user_id, score, total, time_used, timer_limit, timed_out } = body;
  ...
  await db.prepare(
    'INSERT INTO quiz_scores (user_id, score, total, time_used, timer_limit, timed_out) VALUES (?, ?, ?, ?, ?, ?)'
  ).bind(user_id || '', score, total, time_used || 0, timer_limit || 90, timed_out || 0).run();
