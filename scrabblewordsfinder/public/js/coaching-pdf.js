...
async function generateCoachingPDF(opts) {
  ...
  // Top accent bar (full width colour band)
  var accentColors = { quiz: [59, 130, 246], rack: [245, 158, 11], anagram: [244, 63, 94], cab: [249, 115, 22] };
  var accent = accentColors[type] || [34, 197, 94];
  ...
  try {
    ...
    if (logoImg) {
      doc.addImage(logoImg, 'PNG', margin, y + 2, 14, 14);
    }
  } catch {}
  ...
  doc.setFontSize(11);
  doc.setTextColor(34, 197, 94); // emerald brand green
  doc.text('ScrabbleWordsFinder.com', margin + 17, y + 6);

  // Report title
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(18);
  doc.setTextColor(30, 30, 30);
  doc.text('Lex Coaching Report', margin + 17, y + 13);

  // Game type badge (right-aligned)
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9);
  doc.setTextColor(accent[0], accent[1], accent[2]);
  doc.text(typeLabel.toUpperCase(), pageW - margin, y + 6, { align: 'right' });

  // Tagline
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  doc.setTextColor(120, 120, 120);
  doc.text('Free \u2022 Instant \u2022 No Sign-up', pageW - margin, y + 11, { align: 'right' });

  y += 20;

  // Divider line (accent coloured)
  ...
  doc.text('www.scrabblewordsfinder.com', pageW - margin, y, { align: 'right' });
  y += 6;
  ...
  for (var p = 1; p <= totalPages; p++) {
    ...
    doc.text('ScrabbleWordsFinder.com', margin, 289);
    doc.setFont('helvetica', 'italic');
    doc.text(
      'Generated by Lex AI Coach \u2022 Free \u2022 Instant \u2022 No Sign-up',
      pageW / 2, 289, { align: 'center' }
    );
    doc.text('Page ' + p + '/' + totalPages, pageW - margin, 289, { align: 'right' });
    // Bottom accent bar
    doc.setFillColor(accent[0], accent[1], accent[2]);
