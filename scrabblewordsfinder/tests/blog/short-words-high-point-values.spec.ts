...
const PAGE = '/blog/short-words-high-point-values/';

test.describe('Short Words High Point Values — Positive', () => {
  ...
  test('first question mentions highest-scoring 2-letter word', async ({ page }) => {
    await page.goto(PAGE);
    const scripts = await page.locator('script[type="application/ld+json"]').allTextContents();
    const faq = scripts.find(s => s.includes('FAQPage'));
    const parsed = JSON.parse(faq!);
    expect(parsed.mainEntity[0].name).toContain('2-letter');
  });
...
test.describe('Short Words High Point Values — Negative', () => {
