...
const PAGE = '/blog/long-words-low-point-values/';

test.describe('Long Words Low Point Values — Positive', () => {
  ...
  test('first question mentions low-point tiles and bingos', async ({ page }) => {
    await page.goto(PAGE);
    const scripts = await page.locator('script[type="application/ld+json"]').allTextContents();
    const faq = scripts.find(s => s.includes('FAQPage'));
    const parsed = JSON.parse(faq!);
    expect(parsed.mainEntity[0].name.toLowerCase()).toContain('bingo');
  });
...
test.describe('Long Words Low Point Values — Negative', () => {
