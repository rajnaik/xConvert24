...
const PAGE = '/blog/words-ending-with-c/';

test.describe('Words Ending With C — Positive', () => {
  ...
  test('first question mentions words ending in C', async ({ page }) => {
    await page.goto(PAGE);
    const scripts = await page.locator('script[type="application/ld+json"]').allTextContents();
    const faq = scripts.find(s => s.includes('FAQPage'));
    const parsed = JSON.parse(faq!);
    expect(parsed.mainEntity[0].name.toLowerCase()).toContain('end');
    expect(parsed.mainEntity[0].name).toContain('C');
  });
...
test.describe('Words Ending With C — Negative', () => {
