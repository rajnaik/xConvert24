...
const PAGE = '/blog/words-ending-in-ex/';

test.describe('Words Ending in EX — Positive', () => {
  ...
  test('first question mentions highest-scoring words ending in EX', async ({ page }) => {
    await page.goto(PAGE);
    const scripts = await page.locator('script[type="application/ld+json"]').allTextContents();
    const faq = scripts.find(s => s.includes('FAQPage'));
    const parsed = JSON.parse(faq!);
    expect(parsed.mainEntity[0].name.toLowerCase()).toContain('highest-scoring');
  });
...
test.describe('Words Ending in EX — Negative', () => {
