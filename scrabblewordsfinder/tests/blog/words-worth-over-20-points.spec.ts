...
const PAGE = '/blog/words-worth-over-20-points/';

test.describe('Words Worth Over 20 Points — Positive', () => {
  ...
  test('first question mentions highest base score', async ({ page }) => {
    await page.goto(PAGE);
    const scripts = await page.locator('script[type="application/ld+json"]').allTextContents();
    const faq = scripts.find(s => s.includes('FAQPage'));
    const parsed = JSON.parse(faq!);
    expect(parsed.mainEntity[0].name.toLowerCase()).toContain('highest');
  });
...
test.describe('Words Worth Over 20 Points — Negative', () => {
